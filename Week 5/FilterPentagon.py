...
frame = cv2.imread ('collage.png')
edges = cv2.Canny(frame,100,200) # This uses the canny edge detector. 
#The 100 and 200 are rather arbitrary parameters; the second should be larger than the first,
 #play around to see what numbers work best for each image.

# Load another pentagon from a template
pentagon = cv2.imread('pentagon.png')
...
for contour in contours:
    if cv2.contourArea(contour)>100:
        ...
        print(delta)
        if (np.abs(delta)<0.001): #0.001 is our threshold   #change this value to refine
            print(np.abs(delta))
